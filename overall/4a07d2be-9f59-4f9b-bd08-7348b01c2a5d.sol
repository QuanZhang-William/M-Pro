...
interface TransferHook {
     ...
     * @param tokenAddress ether address of the token contract
     * @param tokenAmount uint256 the amount of token you want to transfer
     * @return No return, revert if error occurred
     */
...
*/
contract StandardSecurityToken is ERC20, Ownable {
    event Mint(address indexed to, uint256 value);
    event Burn(address indexed from, uint256 value);

    /*
	* Name and symbol are to be hard-coded for each individual security token contract
    */
    ...
    constructor(uint256 totalSupply, address transferControllerAddr, address codeDefinitionsAddr, address transferHookAddr) public {
        require(transferControllerAddr != address(0));
        require(codeDefinitionsAddr != address(0));
        require(transferHookAddr != address(0));
    ...
    */
    function transfer(address to, uint256 value) public returns (bool) {
        ...
        require(detectTransferRestriction(msg.sender, to, msg.sender, value) == 0);
    ...
    */
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        // Check for transferrability among white-listed individuals only
        require(detectTransferRestriction(from, to, msg.sender, value) == 0);
    ...
    /**
    * @dev Owner of the security token contract (can be another contract) can mint to a specific address
    */
    ...
    /**
    * @dev Owner of the security token contract (can be another contract) can burn balance from a specific address
    */
    ...
    *
    */
    function detectTransferRestriction(address from, address to, address initiator, uint256 value) public view returns (uint256) {
    ...
    */
    function messageForTransferRestriction(uint256 restrictionCode) public view returns (string memory) {
    ...
    */
    function setTransferController(address transferControllerAddr) external onlyOwner {
        require(transferControllerAddr != address(0));
    ...
    */
    function setCodeDefinitions(address codeDefinitionsAddr) external onlyOwner {
        require(codeDefinitionsAddr != address(0));
    ...
    */
    function setTransferHook(address transferHookAddr) external onlyOwner {
        require(transferHookAddr != address(0));
    ...
    /**
    * @notice explicitly forbid eth funding
    */
